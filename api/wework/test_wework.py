...
def ceart_list():
    list = [(random.randint(11111, 99999), "波波", str(random.randint(17100000000, 17999999999))) for x in range(2)]
    print(list)
    return list
class TestWework:
    ...
    def test_delete(self, token, userid='bibi1'):
        """
        请求方式：GET（HTTPS）
        ...
        print(res.json())
        return res.json()
    @pytest.mark.parametrize("userid,mobile,name",[("bibi3","13800139123","星咖特购3")])
    def test_wework(self, token, userid, mobile, name):
        ...
        assert self.test_update(token, userid, '伍佰1', '150010145678')['errmsg'] == 'updated'
        assert self.test_delete(token, userid)['errmsg'] == 'deleted'
        assert self.test_get(token, userid)['errcode'] == 60111

    def test_add(self):
        print(61.6 + 71.76 + 17.88 * 2 + 16.95 + 118.8 + 186 + 9.6 * 2)
